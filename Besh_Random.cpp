...
void Besh_Random::pinky_ran_move(Sprite & pink, int map[][50], int speed){
	...
	if ((map[pinky_x][pinky_y] == 1 || cn % 20 == 0) && !(xmod % 32) && !(ymod % 32)) {
		if ((pinkyx > 0 || pinkyx < 0) && !pinkyy) {
			if (vary) {
				if (map[p_x(-32, pink)][p_y(0, pink)] != 1) // arriba
					pinkyx = 0, pinkyy = -speed;

				else if (map[p_x(32, pink)][p_y(0, pink)] != 1) // abajo
					pinkyx = 0, pinkyy = speed;

				else if (map[p_x(0, pink)][p_y(32, pink)] != 1) // derecha
					pinkyx = speed, pinkyy = 0;

				else if (map[p_x(0, pink)][p_y(-32, pink)] != 1) // izquierda
					pinkyx = -speed, pinkyy = 0;
			}

			else {
				if (map[p_x(32, pink)][p_y(0, pink)] != 1) // abajo
					pinkyx = 0, pinkyy = speed;

				else if (map[p_x(-32, pink)][p_y(0, pink)] != 1) // arriba
					pinkyx = 0, pinkyy = -speed;
		...
		else if (!pinkyx && (pinkyy > 0 || pinkyy < 0)) { // abajo arriba

			if (vary) {
				if (map[p_x(0, pink)][p_y(-32, pink)] != 1) // izquierda
					pinkyx = -speed, pinkyy = 0;
				...
				if (map[p_x(0, pink)][p_y(32, pink)] != 1) // derecha
					pinkyx = speed, pinkyy = 0;
